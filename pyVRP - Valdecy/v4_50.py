...
coordinates     = pd.read_csv('VRP-V4-50-Coordinates.txt', sep = '\t') 
parameters      = pd.read_csv('VRP-V4-50-Parameters.txt', sep = '\t')
...
capacity      = [150, 200, 250]  # Capacity of Vehicle A = 9; Capacity of Vehicle B = 8
...
fleet_size    = [1 , 1 , 2]  # An Empty List, e.g  fleet_size = [ ], It Means that the Fleet is Infinite. Non-Empty List, e.g  fleet_size = [15, 7], Means that there are available 15 vehicles of type A and 7 vehicles of type B
...
ga_report.to_csv('VRP-V4-50-Report.csv', sep = ';', index = False)