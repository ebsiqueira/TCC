...
coordinates     = pd.read_csv('VRP-V10-78-Coordinates.txt', sep = '\t') 
parameters      = pd.read_csv('VRP-V10-78-Parameters.txt', sep = '\t')
...
capacity      = [60, 80, 120, 140]  # Capacity of Vehicle A = 9; Capacity of Vehicle B = 8
velocity      = [1 , 1 , 1 , 1]  # Average Velocity of Vehicle A = 50; Average Velocity of Vehicle B = 70. The Average Velocity Value is Used as a Constant that Divides the Distance Matrix.
fleet_size    = [3 ,  2 , 3 , 2]  # An Empty List, e.g  fleet_size = [ ], It Means that the Fleet is Infinite. Non-Empty List, e.g  fleet_size = [15, 7], Means that there are available 15 vehicles of type A and 7 vehicles of type B
...
ga_report.to_csv('VRP-V10-78-Report.csv', sep = ';', index = False)