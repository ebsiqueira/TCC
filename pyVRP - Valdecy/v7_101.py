...
coordinates     = pd.read_csv('VRP-V7-101-Coordinates.txt', sep = '\t') 
parameters      = pd.read_csv('VRP-V7-101-Parameters.txt', sep = '\t')
...
# Parameters - Vehicle
vehicle_types = 4          # Quantity of Vehicle Types. In this examples there are 2 Types of Vehicles: A (  or 0) and B ( or 1)
fixed_cost    = [1 , 1 , 1 , 1]  # Fixed Cost for Vehicle A = 12; Fixed Cost for Vehicle B = 25
variable_cost = [1 ,  1 , 1 , 1]  # Variable Cost for Vehicle A = 0.5; Variable Cost for Vehicle B = 1
capacity      = [400 ,  600, 800, 1000]  # Capacity of Vehicle A = 9; Capacity of Vehicle B = 8
velocity      = [70 , 70 , 70 , 70]  # Average Velocity of Vehicle A = 50; Average Velocity of Vehicle B = 70. The Average Velocity Value is Used as a Constant that Divides the Distance Matrix.
fleet_size    = [1 ,  2 , 2 , 2]  # An Empty List, e.g  fleet_size = [ ], It Means that the Fleet is Infinite. Non-Empty List, e.g  fleet_size = [15, 7], Means that there are available 15 vehicles of type A and 7 vehicles of type B
...
ga_report.to_csv('VRP-V7-101-Report.csv', sep = ';', index = False)