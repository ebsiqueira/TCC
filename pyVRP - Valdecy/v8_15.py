...
coordinates     = pd.read_csv('VRP-V8-15-Coordinates.txt', sep = '\t') 
parameters      = pd.read_csv('VRP-V8-15-Parameters.txt', sep = '\t')
...
fixed_cost    = [0 , 0]  # Fixed Cost for Vehicle A = 02; Fixed Cost for Vehicle B = 25
variable_cost = [1 ,  1]  # Variable Cost for Vehicle A = 0.5; Variable Cost for Vehicle B = 1
capacity      = [30, 40]  # Capacity of Vehicle A = 9; Capacity of Vehicle B = 8
velocity      = [1 , 1 ]  # Average Velocity of Vehicle A = 50; Average Velocity of Vehicle B = 70. The Average Velocity Value is Used as a Constant that Divides the Distance Matrix.
fleet_size    = [4 ,  4]  # An Empty List, e.g  fleet_size = [ ], It Means that the Fleet is Infinite. Non-Empty List, e.g  fleet_size = [15, 7], Means that there are available 15 vehicles of type A and 7 vehicles of type B

# Parameters - GA
penalty_value   = 10000    # GA Target Function Penalty Value for Violating the Problem Constraints
population_size = 1000       # GA Population Size
...
ga_report.to_csv('VRP-V8-15-Report.csv', sep = ';', index = False)